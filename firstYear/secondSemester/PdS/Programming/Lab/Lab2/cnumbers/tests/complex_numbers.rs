...
pub fn test_try_from_f64() {
    ...
    let a_cn: Result<ComplexNumber, _> = a.try_into();
    assert_eq!(a_cn, Ok(ComplexNumber::new(1.0, 1.0)));

    let b_cn: Result<ComplexNumber, _> = b.try_into();
    assert!(b_cn.is_err());
}
...
pub fn test_deque() {
    ...
    let c = ComplexNumber::new(3.0, 6.0);
    let d = ComplexNumber::new(4.0, 8.0);
    let e = ComplexNumber::new(5.0, 10.0);
    let f = ComplexNumber::new(6.0, 12.0);
    let g = ComplexNumber::new(7.0, 14.0);
    ...
    let index = deque.binary_search(&a);

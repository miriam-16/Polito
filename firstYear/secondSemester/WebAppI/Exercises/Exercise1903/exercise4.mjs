...
function Answer(id, text, email, date, score = 0) {
  ...
  function QuestionList(){
    ...
    this.getUserId = function(email){
      return new Promise((resolve, reject) => {
        const sql = `select u.id from "user" u where u.email = ?`
        db.get(sql, [email], (err, row) => {
          if(err) reject(err)
          else resolve(Number(row.id))
        }) 
      })
    }
    ...
    this.addQuestion = function(q){
      //return the id field incremented in order to return always a value
      const userEmail = q.email

      const userIdPromise = this.getUserId(userEmail)
      
      return userIdPromise.then((userId) => { //if execution of userIdPromise succeeds, userId value is the id of the corresponding email
